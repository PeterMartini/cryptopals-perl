...
sub decrypt_cbc {
  ...
  my $lastchunk = pack("H*","00"x16);
  my $text;
  for my $chunk ($data =~ /(.{16})/mgs) {
    my $decrypted = $m->decrypt($chunk, $key) ^ $lastchunk;
    my $last = ord(substr($decrypted, 15, 1));
    if ($last < 16 && substr($decrypted, 16 - $last, $last) eq (chr($last) x $last)) {
      $decrypted = substr($decrypted, 0, 16 - $last);
    }
    $text .= $decrypted;
    $lastchunk = $chunk;
  }
...
is <$funkymusic>, decrypt_cbc($data,"YELLOW SUBMARINE");
close($funkymusic);
